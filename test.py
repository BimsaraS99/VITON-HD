...
def get_opt():
    ...
    parser.add_argument('--dataset_dir', type=str, default='openpose_gpu/main_inputs/')
...
def test(opt, seg, gmm, alias):
    ...
    with torch.no_grad():
        for i, inputs in enumerate(test_loader.data_loader):
            ...
            img_agnostic = inputs['img_agnostic'].to("cpu")
            parse_agnostic = inputs['parse_agnostic'].to("cpu")
            pose = inputs['pose'].to("cpu")
            c = inputs['cloth']['unpaired'].to("cpu")
            cm = inputs['cloth_mask']['unpaired'].to("cpu")
            ...
            seg_input = torch.cat((cm_down, c_masked_down, parse_agnostic_down, pose_down, gen_noise(cm_down.size()).to("cpu")), dim=1)

            parse_pred_down = seg(seg_input)
            parse_pred = gauss(up(parse_pred_down))
            parse_pred = parse_pred.argmax(dim=1)[:, None]

            parse_old = torch.zeros(parse_pred.size(0), 13, opt.load_height, opt.load_width, dtype=torch.float).to("cpu")
            ...
            parse = torch.zeros(parse_pred.size(0), 7, opt.load_height, opt.load_width, dtype=torch.float).to("cpu")
